...
class MainWindow:
    def __init__(self):
        ...
        self.button1.grid(
            row=5, column=5, columnspan=2, ipady=0, pady=0
        )  # 使用 grid 布局
        ...
        self.button2.grid(
            row=5, column=4, columnspan=4, ipady=0, pady=0, sticky="e"
        )  # 使用 grid 布局
        ...
        self.button3.grid(
            row=5, column=4, columnspan=4, ipady=0, pady=0, sticky="w"
        )  # 使用 grid 布局

        # 在主窗口添加标签，显示文字
        self.label = tk.Label(
            self.root,
            text="一种面向荧光染色牛肌肉组织切片扫描\n图像的肌纤维信息提取系统",
            font=("微软雅黑", 34),
        )
...
class SubWindow1:
    def __init__(self):
        ...
        label = tk.Label(
            self.root,
            text="一种面向荧光染色牛肌肉组织切片扫描图像的肌纤维信息提取系统",
            font=("微软雅黑", 20),
        )
        ...
        self.file_entry = tk.Entry(
            self.root, textvariable=self.file_var, width=30, font=("微软雅黑", 16)
        )
        self.file_entry.grid(
            row=2, column=1, columnspan=1, padx=0, ipadx=0, pady=0, ipady=0, sticky="nw"
        )
        ...
        self.path_entry1 = tk.Entry(
            self.root, textvariable=self.path_var1, width=30, font=("微软雅黑", 16)
        )
        self.path_entry1.grid(
            row=3, column=1, columnspan=1, padx=0, ipadx=0, pady=0, ipady=0, sticky="nw"
        )
        ...
        button1 = tk.Button(
            ...
            fg="white",
            font=("微软雅黑", 12),
            command=lambda: self.use_cellpose_s(
                self.selected_file, self.selected_path1
            ),
        )
        ...
        self.numeric_entry1 = tk.Entry(
            self.root,
            textvariable=self.numeric_var1,
            validate="key",
            font=("微软雅黑", 14),
            width=19,
        )
        ...
        self.path_entry2 = tk.Entry(
            self.root, textvariable=self.path_var2, width=30, font=("微软雅黑", 16)
        )
        self.path_entry2.grid(
            row=2, column=6, columnspan=1, padx=0, ipadx=0, pady=0, ipady=0, sticky="nw"
        )
        ...
        self.path_entry3 = tk.Entry(
            self.root, textvariable=self.path_var3, width=30, font=("微软雅黑", 16)
        )
        self.path_entry3.grid(
            row=3, column=6, columnspan=1, padx=0, ipadx=0, pady=0, ipady=0, sticky="nw"
        )
        ...
        button2 = tk.Button(
            ...
            fg="white",
            font=("微软雅黑", 12),
            command=lambda: self.use_cellpose_m(
                self.selected_path2, self.selected_path3
            ),
        )
        ...
        self.numeric_entry2 = tk.Entry(
            self.root,
            textvariable=self.numeric_var2,
            validate="key",
            font=("微软雅黑", 14),
            width=19,
        )
        ...
        self.clear_button.grid(
            row=6, column=5, columnspan=2, sticky="e"
        )  # 设置 sticky 为 "ew" 表示居中对齐
    ...
    def use_cellpose_s(self, file, path1, mod=1):
        try:
            ...
            resultarr, cell_arear, img_arear, diameterr, pr1, pr2 = use_cellpose(
                rp, path1, img=r, pixel_size=self.pixel_size
            )
            resultarg, cell_areag, img_areag, diameterg, pg1, pg2 = use_cellpose(
                gp, path1, img=g, pixel_size=self.pixel_size
            )
            resultarb, cell_areab, img_areab, diameterb, pb1, pb2 = use_cellpose(
                bp, path1, img=b, pixel_size=self.pixel_size
            )
            resultarp, cell_areap, img_areap, diameterp, pp1, pp2 = use_cellpose(
                pp, path1, img=p, pixel_size=self.pixel_size
            )
            ...
            try:
                ...
                filer.write(
                    f"{round(resultarr / resultar_all, 2)}"
                    + "%\t\t"
                    + f"{round(cell_arear / img_arear, 2)}"
                    + "%\t\t\t"
                    + f"{round(diameterr / resultar[0], 2)}"
                    + "um\t\t"
                    + f"{round(cell_arear, 2)}"
                    + "um^2\n"
                )
            except:
                filer.write(
            ...
            try:
                ...
                fileg.write(
                    f"{round(resultarg / resultar_all, 2)}"
                    + "%\t\t"
                    + f"{round(cell_areag / img_areag, 2)}"
                    + "%\t\t\t"
                    + f"{round(diameterg / resultar[1], 2)}"
                    + "um\t\t"
                    + f"{round(cell_areag, 2)}"
                    + "um^2\n"
                )
            except:
                fileg.write(
            ...
            try:
                ...
                fileb.write(
                    f"{round(resultarb / resultar_all, 2)}"
                    + "%\t\t"
                    + f"{round(cell_areab / img_areab, 2)}"
                    + "%\t\t\t"
                    + f"{round(diameterb / resultar[2], 2)}"
                    + "um\t\t"
                    + f"{round(cell_areab, 2)}"
                    + "um^2\n"
                )
            except:
                fileb.write(
            ...
            try:
                ...
                filep.write(
                    f"{round(resultarp / resultar_all, 2)}"
                    + "%\t\t"
                    + f"{round(cell_areap / img_areap, 2)}"
                    + "%t\t\t\t"
                    + f"{round(diameterp / resultar[3], 2)}"
                    + "um\t\t"
                    + f"{round(cell_areap, 2)}"
                    + "um^2\n"
                )
            except:
                filep.write(
            ...
            for option in selected_options:
                match option:
                    case "Type I":
                        Label = tk.Label(
                            self.table_frame,
                            text=f"{round((resultar[i] / resultar_all) * 100, 2)}",
                            width=19,
                            height=2,
                            bg="#E9EBF5",
                            font=("等线", 12, "bold"),
                        ...
                        Label = tk.Label(
                            self.table_frame,
                            text=f"{round((cell_area[i] / img_area[i]) * 100, 2)}",
                            width=19,
                            height=2,
                            bg="#CFD5EA",
                            font=("等线", 12, "bold"),
    ...
    def __init__(self):
        ...
        def check_options():
            ...
            for combo in combo_boxes:
                ...
                if selected_option in selected_options:
                    show_error(
                        "Error", "Duplicate option found. Please select unique options."
                    )
                    return
        ...
        for i in range(4):
            ...
            combo_box = ttk.Combobox(
                self.root,
                values=["Type I", "Type IIA", "Type IIX", "Type IIA + IIX"],
                font=("等线", 16, "bold"),
                width=15,
                height=20,
            )
