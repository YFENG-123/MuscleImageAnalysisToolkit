...
def getResults(img0, masks, pixel_size, color=[1, 0, 0]):
    ...
    for i in range(len(outY)):
        imag_with_boundaries = cv2.circle(
            imag_with_boundaries, (outX[i], outY[i]), radius=1, color=color, thickness=2
        )
...
def use_cellpose(imgpath, resfilepath, pixel_size=10.0):
    ...
    # 展示图片
    # cv2.imshow("cell_boundary", cell_boundary)  # removed: no GUI support
    # cv2.waitKey()  # removed: no GUI support
